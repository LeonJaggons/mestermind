    ...
    const fetchConversations = async () => {
      ...
      try {
        ...
        const conversationsArray = Array.from(conversationMap.values())
          .filter(conv => conv.unread_count > 0 && !conv.is_archived)
        ...
        <main className="absolute inset-0 overflow-y-auto">
          <div className="flex flex-col h-full overflow-x-hidden overflow-y-auto bg-white">
            ...
            <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-4">
              <h1 className="text-2xl font-bold text-gray-900 mb-1">{t("pro.messages.unread.title")}</h1>
              <p className="text-sm text-gray-600">{t("pro.messages.unread.subtitle")}</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="flex justify-center items-center text-center flex-grow py-12">
                <div className="max-w-md">
                  ...
                  <h3 className="font-bold text-lg mb-2">{t("pro.messages.unread.noUnread")}</h3>
                  <p className="text-gray-600 text-sm leading-relaxed">
                    {t("pro.messages.unread.noUnreadDescription")}
                  </p>
                </div>
              </div>
            )}
