...
export default function NoStatusMessagesPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      ...
      try {
        ...
        // Get accepted invitations (hired jobs) - exclude these from "not scheduled"
        const invitationsResponse = await fetch(`${API_BASE_URL}/api/v1/invitations?pro_profile_id=${proProfile.id}&status=accepted`);
        const acceptedInvitations = invitationsResponse.ok ? await invitationsResponse.json() : [];
        const hiredJobIds = new Set(acceptedInvitations.map((inv: any) => inv.job_id));
        ...
        for (const message of messages) {
          if (!conversationMap.has(message.job_id)) {
            ...
            if (jobResponse.ok) {
              ...
              // Exclude jobs where the pro was hired (accepted invitation)
              if (hiredJobIds.has(message.job_id)) {
                continue;
              }
              
              // Filter: only show jobs with status "draft" or "open" (not in_progress, completed, or cancelled)
        ...
        <main className="absolute inset-0 overflow-y-auto">
          <div className="flex flex-col h-full overflow-x-hidden overflow-y-auto bg-white">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-4">
              <h1 className="text-2xl font-bold text-gray-900 mb-1">{t("pro.messages.statusFilters.notScheduledYet")}</h1>
              <p className="text-sm text-gray-600">{t("pro.messages.statusFilters.notScheduledYetDescription")}</p>
            </div>
