...
export default function StatusDoneMessagesPage() {
  const { user } = useAuth();
        ...
        for (const message of messages) {
          if (!conversationMap.has(message.job_id)) {
            ...
            if (jobResponse.ok) {
              ...
              // Filter: only show jobs with status "completed"
        ...
        <main className="absolute inset-0 overflow-y-auto">
          <div className="flex flex-col h-full overflow-x-hidden overflow-y-auto bg-white">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-4">
              <h1 className="text-2xl font-bold text-gray-900 mb-1">{t("pro.messages.statusFilters.jobDone")}</h1>
              <p className="text-sm text-gray-600">{t("pro.messages.statusFilters.jobDoneDescription")}</p>
            </div>
