...
export default function StarredMessagesPage() {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      ...
      try {
        ...
        // Filter messages to only starred conversations
        const starredMessages = messages.filter(msg => starredJobIds.includes(msg.job_id));
  ...
  const handleUnstar = async (jobId: number, e: React.MouseEvent) => {
    e.stopPropagation();
        ...
        <main className="absolute inset-0 overflow-y-auto">
          <div className="flex flex-col h-full overflow-x-hidden overflow-y-auto bg-white">
            ...
            <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-4">
              <h1 className="text-2xl font-bold text-gray-900 mb-1">{t("pro.messages.starred.title")}</h1>
              <p className="text-sm text-gray-600">{t("pro.messages.starred.subtitle")}</p>
            </div>
            ...
            {loading ? (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3 mb-1">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <button
                              onClick={(e) => handleUnstar(conversation.job_id, e)}
                              disabled={starringJobId === conversation.job_id}
                              className="p-1.5 text-yellow-500 hover:text-yellow-600 hover:bg-gray-100 rounded transition-colors"
                              title={t("pro.messages.unstar")}
                            >
              ...
              <div className="flex justify-center items-center text-center flex-grow py-12">
                <div className="max-w-md">
                  ...
                  <h3 className="font-bold text-lg mb-2">{t("pro.messages.starred.noStarred")}</h3>
                  <p className="text-gray-600 text-sm leading-relaxed">
                    {t("pro.messages.starred.noStarredDescription")}
                  </p>
                </div>
              </div>
            )}
