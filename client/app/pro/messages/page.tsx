...
export default function MessagesPage() {
    ...
    try {
      ...
      if (response.ok) {
        setConversations(prev => 
          prev.map(conv => 
            conv.job_id === jobId ? { ...conv, is_archived: true } : conv
          )
        );
      }
    } catch (error) {
  ...
  return (
    <div className="flex relative bg-white rounded-lg shadow-sm">
      {/* Sidebar Navigation */}
      <MessagesSidebar />

      {/* Main Content Area */}
      <div className="flex-auto border-l border-gray-200">
        <main className="h-full overflow-y-auto">
          <div className="flex flex-col h-full overflow-x-hidden overflow-y-auto bg-white">
            {/* Enhanced Header */}
            <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-4">
              <h1 className="text-2xl font-bold text-gray-900 mb-1">{t("pro.messages.title")}</h1>
              <p className="text-sm text-gray-600">{t("pro.messages.subtitle")}</p>
            </div>
            ...
            {loading ? (
              <div className="grow">
                <div className="divide-y divide-gray-100 animate-pulse">
                  {Array.from({ length: 6 }).map((_, idx) => (
                    <div
                      key={idx}
                      className="px-6 py-5 border-l-4 border-transparent"
                    >
                        ...
                        <div className="flex items-start justify-between gap-3 mb-1">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {conversation.unread_count > 0 && (
                                <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[hsl(var(--primary))] text-white text-xs font-bold shrink-0">
              ...
              <div className="flex justify-center items-center text-center grow py-12">
                <div className="max-w-md">
                  ...
                  <h3 className="font-bold text-lg mb-2">{t("pro.messages.noMessagesYet")}</h3>
                  <p className="text-gray-600 text-sm leading-relaxed">
                    {t("pro.messages.noMessagesDescription")}
                  </p>
                </div>
              </div>
            )}
